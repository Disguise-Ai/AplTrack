...
import { View, StyleSheet, useColorScheme, ScrollView, TouchableOpacity, Alert, Linking, Modal } from 'react-native';
...
export default function SettingsScreen() {
  ...
  const handlePurchase = async () => { if (!monthlyPackage) { Alert.alert('Error', 'Subscription not available. Please try again later.'); return; } try { await purchase(monthlyPackage); setShowPaywall(false); Alert.alert('Success', 'Welcome to AplTrack Premium!'); } catch (error: any) { if (error.message !== 'Purchase cancelled') Alert.alert('Error', error.message || 'Purchase failed. Please try again.'); } };
  const handleRestore = async () => { try { await restore(); Alert.alert('Success', 'Purchases restored successfully!'); } catch (error: any) { Alert.alert('Error', error.message || 'Failed to restore purchases.'); } };
  const openLink = (url: string) => { Linking.openURL(url); };

  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}><Text variant="title" weight="bold">Settings</Text></View>
      ...
      <Modal visible={showPaywall} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowPaywall(false)}>
        <SafeAreaView style={[styles.paywallContainer, { backgroundColor: colors.background }]}>
          <View style={styles.paywallHeader}><TouchableOpacity onPress={() => setShowPaywall(false)}><Ionicons name="close" size={28} color={colors.text} /></TouchableOpacity></View>
          <ScrollView contentContainerStyle={styles.paywallContent}>
            <View style={[styles.paywallIcon, { backgroundColor: colors.primary }]}><Ionicons name="diamond" size={48} color="#FFFFFF" /></View>
            ...
            <View style={styles.features}><PaywallFeature icon="sync-outline" title="Real-time Analytics Sync" description="Get live updates from App Store Connect" /><PaywallFeature icon="chatbubbles-outline" title="Unlimited AI Chat" description="Marketing and sales advice on demand" /><PaywallFeature icon="people-outline" title="Community Access" description="Post and engage with other founders" /><PaywallFeature icon="notifications-outline" title="Push Notifications" description="Instant alerts for downloads and revenue" /></View>
            <View style={styles.priceContainer}><Text variant="title" weight="bold">{Config.SUBSCRIPTION_PRICE}</Text><Text variant="caption" color="secondary">Cancel anytime</Text></View>
            <Button title={purchasing ? 'Processing...' : 'Subscribe Now'} onPress={handlePurchase} loading={purchasing} disabled={!monthlyPackage || purchasing} size="large" style={styles.subscribeButton} />
            <TouchableOpacity onPress={handleRestore} style={styles.restoreButton}><Text variant="label" color="secondary">Restore Purchases</Text></TouchableOpacity>
            <Text variant="caption" color="secondary" align="center" style={styles.disclaimer}>Payment will be charged to your Apple ID account. Subscription automatically renews unless canceled at least 24 hours before the end of the current period.</Text>
          </ScrollView>
        </SafeAreaView>
      </Modal>
    </SafeAreaView>
...
const styles = StyleSheet.create({ container: { flex: 1 }, scrollContent: { padding: 16, paddingBottom: 32 }, header: { marginBottom: 24 }, profileCard: { marginBottom: 16 }, profileContent: { flexDirection: 'row', alignItems: 'center' }, avatar: { width: 56, height: 56, borderRadius: 28, alignItems: 'center', justifyContent: 'center' }, profileInfo: { flex: 1, marginLeft: 16 }, premiumBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12 }, subscriptionCard: { marginBottom: 24 }, subscriptionContent: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 }, crownIcon: { width: 48, height: 48, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12 }, subscriptionInfo: { flex: 1 }, upgradeButton: { alignSelf: 'stretch' }, section: { marginBottom: 24 }, sectionTitle: { marginBottom: 8, marginLeft: 4 }, sectionCard: { overflow: 'hidden' }, settingsItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 16, borderBottomWidth: StyleSheet.hairlineWidth }, settingsLabel: { flex: 1, marginLeft: 12 }, signOutButton: { marginTop: 8 }, version: { marginTop: 24 }, paywallContainer: { flex: 1 }, paywallHeader: { flexDirection: 'row', justifyContent: 'flex-end', padding: 16 }, paywallContent: { padding: 24, alignItems: 'center' }, paywallIcon: { width: 96, height: 96, borderRadius: 24, alignItems: 'center', justifyContent: 'center', marginBottom: 24 }, paywallTitle: { marginBottom: 8 }, paywallSubtitle: { marginBottom: 32 }, features: { width: '100%', marginBottom: 32 }, featureItem: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 }, featureIcon: { width: 48, height: 48, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 16 }, featureText: { flex: 1 }, priceContainer: { alignItems: 'center', marginBottom: 24 }, subscribeButton: { width: '100%', marginBottom: 16 }, restoreButton: { padding: 12 }, disclaimer: { marginTop: 16, paddingHorizontal: 16 } });
