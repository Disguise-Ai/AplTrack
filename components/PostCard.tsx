...
// Move formatDate outside component for reuse
...
export const PostCard = React.memo(function PostCard({ post, onPress, onLike, isLiked = false }: PostCardProps) {
  ...
  // Memoize author initial
  const authorInitial = useMemo(
    () => post.profile?.full_name?.[0]?.toUpperCase() || 'A',
  ...
  return (
    <Card style={styles.card} onPress={onPress}>
      <View style={styles.header}>
        ...
        {post.category && (
          <View style={[styles.category, { backgroundColor: colors.primary + '15' }]}>
      ...
      <Text variant="body" color="secondary" numberOfLines={3} style={styles.content}>
        {post.content}
      </Text>
      <View style={styles.footer}>
        <TouchableOpacity style={styles.action} onPress={onLike}>
          ...
          />
          <Text variant="caption" color={isLiked ? 'error' : 'secondary'} style={styles.actionText}>
            {post.likes_count}
          </Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  category: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
